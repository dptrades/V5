    ...
    try {
        ...
        try {
            ...
            if (terminalRes.ok) {
                ...
                if (terminalScore < 40) {
                    console.log(`[Auto-Trade] Terminal Guardrail blocked trades. Score: ${terminalScore}`);
                    return NextResponse.json({
                        error: 'Terminal Guardrail triggered: Total score is below 40. Market conditions are too poor for new trades.',
                        terminalScore,
                        vixLevel
                    }, { status: 400 });
                }
                ...
                if (vixLevel > 25) {
                    console.log(`[Auto-Trade] Terminal Guardrail blocked trades. VIX: ${vixLevel}`);
                    return NextResponse.json({
                        error: 'Terminal Guardrail triggered: VIX is elevated (> 25). Volatility is too high for new trades.',
                        terminalScore,
                        vixLevel
                    }, { status: 400 });
                }
            } else {
                console.warn('[Auto-Trade] Failed to fetch terminal guardrail data');
            }
        } catch (e) {
            console.error('[Auto-Trade] Error fetching terminal guardrail:', e);
        }
